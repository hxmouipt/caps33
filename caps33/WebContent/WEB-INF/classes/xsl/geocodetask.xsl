...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:import href="task-layout-nodes.xsl"/>
  ...
  <xsl:variable name="contextPath">
    <xsl:value-of select="//context-path"/>/</xsl:variable>
  <xsl:variable name="common" select="//common-resources-registered"/>
  <xsl:variable name="taskId">
    <xsl:value-of select="//id"/>
  </xsl:variable>
  <xsl:variable name="activeTool">
    <xsl:value-of select="//active-tool"/>
  </xsl:variable>
  ...
  <xsl:variable name="taskCellId">EsriTaskCell_<xsl:value-of select="$taskId"/>
  </xsl:variable>
  <!-- Window Manager Name -->
      ...
      <xsl:when test="count(/task/task-layouts/task-layout) > 0">
        <xsl:for-each select="/task/task-layouts/task-layout">
          ...
          <xsl:element name="div">
            <xsl:attribute name="id"><xsl:value-of select="$layoutTaskCellId"/></xsl:attribute>
            <xsl:if test="$style != '' or @style != ''">
              <xsl:attribute name="style"><xsl:if test="@style != ''"><xsl:value-of select="@style"/>;
                </xsl:if><xsl:if test="$style != '' and position() = 1"><xsl:value-of select="$style"/></xsl:if></xsl:attribute>
            </xsl:if>
  ...
  <xsl:template name="showInWindow">
    ...
    <xsl:variable name="pe">pe_<xsl:value-of select="$id"/>
    </xsl:variable>
    <xsl:variable name="win">win_<xsl:value-of select="$id"/>
    </xsl:variable>
    <xsl:element name="script">
      <xsl:attribute name="language">Javascript</xsl:attribute>
