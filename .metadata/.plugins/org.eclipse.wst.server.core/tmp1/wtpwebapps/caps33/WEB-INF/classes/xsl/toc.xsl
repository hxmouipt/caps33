    ...
    <xsl:variable name="nodeCnt" select="count(/toc/node)"/>
      ...
      <xsl:element name="script">
        ...
        function esriTocInit<xsl:value-of select="$tocId"/>() {
          var <xsl:value-of select="$tocId"/> = new EsriToc("<xsl:value-of select="$tocId"/>", document.getElementById("<xsl:value-of select="$tocCellId"/>"), "<xsl:value-of select="//map-id" />");
        ...
        <xsl:if test="/toc/toc-model-type = 'WebResultsToc'">
